...
def get_price_min_tx(code, end_date=None, count=10, frequency='1d'):  # 分钟线获取
    ...
    df.at[df.index[-1], 'close'] = float(st['data'][code]['qt'][code][3])

    return df


# 腾讯分钟线，还包括涨跌幅
def get_price_and_change_min_tx(code, end_date=None, count=10, frequency='1d'):  # 分钟线获取
    ts = int(frequency[:-1]) if frequency[:-1].isdigit() else 1  # 解析K线周期数
    if end_date:
    ...
    df.at[df.index[-1], 'close'] = latest_price  # 更新最新价格

    # 获取昨日收盘价
    pre_close = float(st['data'][code]['qt'][code][4])

    # 计算涨跌幅
    df['change'] = ((df['close'] - pre_close) / pre_close * 100).round(2)

    # 获取股票名称
    df['name'] = st['data'][code]['qt'][code][1]
    df['code'] = code
    return df
...
if __name__ == '__main__':
    # df=get_price('sh000001',frequency='1m',count=1)      #支持'1d'日, '1w'周, '1M'月
    # print('上证指数日线行情\n',df)

    df = get_price('000001.XSHG', frequency='1m', count=1)  # 支持'1m','5m','15m','30m','60m'
    print('上证指数分钟线\n', df)
